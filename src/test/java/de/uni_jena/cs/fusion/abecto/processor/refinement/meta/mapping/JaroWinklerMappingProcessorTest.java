...
public class JaroWinklerMappingProcessorTest {
	...
	private static RdfGraph FIRST_GRAPH;
	private static RdfGraph SECOND_GRAPH;

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		FIRST_GRAPH = new RdfGraph(JaroWinklerMappingProcessorTest.class
				.getResourceAsStream("JaroWinklerMappingProcessorTest-example1.ttl"));
		SECOND_GRAPH = new RdfGraph(JaroWinklerMappingProcessorTest.class
	...
	public void testComputeMapping() throws Exception {
		...
		Collection<Mapping> mappings = processor.computeMapping(FIRST_GRAPH.getGraph(), SECOND_GRAPH.getGraph());
	...
	public void testComputeResultGraph() throws Exception {
		JaroWinklerMappingProcessor processor = new JaroWinklerMappingProcessor();
		processor.setProperties(Map.of("threshold", 0.90D, "case_sensitive", false));
		processor.addInputGraphsGroups(
				List.of(Collections.singletonList(FIRST_GRAPH), Collections.singletonList(SECOND_GRAPH)));
		RdfGraph result = processor.computeResultGraph();
		List<Triple> triples = new ArrayList<>();
		result.getGraph().find(Node.ANY, Node.ANY, Node.ANY).forEachRemaining(triples::add);
